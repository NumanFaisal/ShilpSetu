...
export default function BuyerLoginScreen() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      await sendOTP(phone);
      router.push({ pathname: '/(auth)/buyer-otp', params: { phone } });
    } catch {
        ...
        >
          <View style={{ marginBottom: 40, gap: 8 }}>
            ...
            <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 15, color: '#56423C', lineHeight: 22 }}>
              Connect with verified artisans and source genuine handmade products at wholesale prices.
            </Text>
          </View>
          ...
          <View style={{ gap: 24 }}>
            <Input
              ...
              maxLength={13}
            />
            <Button label="Send OTP" onPress={handleSendOTP} loading={loading} disabled={phone.length < 10} />
          </View>

          <TouchableOpacity onPress={() => router.push('/(auth)/artisan-login')} style={{ marginTop: 32, alignItems: 'center' }}>
            <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 14, color: '#8A726B' }}>
              Are you an artisan?{' '}
              <Text style={{ color: '#B5502F', fontFamily: 'Inter_500Medium' }}>Sign in as Artisan</Text>
            </Text>
          </TouchableOpacity>
        </ScrollView>
